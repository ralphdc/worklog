...
if(!function_exists('wp_record_ip')):
    function wp_record_ip()
    {
        $redis = new Redis();
        if($conn = $redis->connect('127.0.0.1','6379')){
            $ip = wp_get_ip();
            if(strlen(strval($ip)) > 0){
                $redis->sAdd('set:blog:visitor',$ip);
				$date = date("Y-m-d H:i:s",strtotime('+8 hours'));
				$redis->hSet('hash:blog:visitor',$date,$ip);
            }
        }
    }
endif;
