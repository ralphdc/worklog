...
class EntityEncrypt{
    ...
    private function construct_msg_six()
    {
        ...
        $sidlen = array(strlen(strval($this->sid)));
        
        $sidbin = $this->byte->getbytes(strval($this->sid));
        
        $pwdlen = array(dechex(strlen(strval($this->pwd))));
        
        $pwdbin = array(0x36,0x36,0x36,0x36,0x36,0x36,0x36,0x36);
        $backmachine = $this->machine == 0 ? array(0x00) : array(0x01);
        $merge = array_merge($command,$sidlen,$sidbin,$pwdlen,$pwdbin,$backmachine);
        $response['msg'] = array_merge($command,$sidlen,$sidbin,$pwdlen,$pwdbin,$backmachine);
    ...
    {
        $message = $this->construct_msg_six();
        //求得消息体长度；
