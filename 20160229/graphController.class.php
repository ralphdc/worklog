...
class GraphController extends CommonController{
    ...
    public function __init()
    {
        ...
        //redis初始化；
        $this->sid = session_id();
        $this->topo_hash = "topo:hash:".$this->sid;
    }
    ...
    public function monitor()
    {
        if($_GET['id']){
            ...
            if(isset($info['errorCode']) && $info['errorCode'] == 0){
                ...
                //图标集合；
                $dtype = $this->getDeviceNameList();
                //2016-02-25 添加子拓扑名称；
                $dtype['childtp'] = "子拓扑图";
                $this->assign('dName',json_encode($dtype));
                $this->display();
            }else{
                $ret = array("statusCode"=>"0","message"=>'读取数据有误，请稍后再试！',"navTabId"=>$this->navTab,
                    "rel"=>"","forwardUrl"=>"","callbackType"=>"");
       ...
       if($_POST){
           ...
           //验证设备类型信息；
            ...
            $create = array(
                ...
                'deviceType'     =>I('post.deviceType'),         //设备类型；
            ...
            $nodeInfo = $nodeInfoModel->searchById(array('id'=>$create['nodeid']));
